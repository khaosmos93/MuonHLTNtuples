...
void ReadNtuples_Rate_v3(TString Menu, std::string hltname, bool verbose) {
  TTimeStamp ts_start;
  ...
  TString outputDir = "outputs/Rate_v3";
  ...
  TString outputPath = outputDir+"/Rate_v3_"+Menu+"_"+hltname+"_"+theDate+".root";
  ...
  else if (hltname.find("HLT_OldMu100_v") != std::string::npos ) {
    filter = "hltL3fL1sMu22Or25L1f0L2f10QL3Filtered100Q";
  }
  else if (hltname.find("HLT_TkMu100_v") != std::string::npos ) {
    filter = "hltL3fL1sMu25f0TkFiltered100Q";
  }
  else if (hltname.find("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8_v") != std::string::npos ) {
    filter = "hltL3fL1DoubleMu155fFiltered17";   //FIXME  hltDiMuon178Mass3p8Filtered
  }
  ...
  // delta eta = 0.3
  Double_t EtaBin[10] = {0.0, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 10.0};
  const Int_t nBins = 9;
  TGraph2D* lumi_run_bin[nBins];
  TGraph2D* nvtx_run_bin[nBins];
  for(Int_t iBin=0; iBin < nBins; ++iBin) {
    lumi_run_bin[iBin] = new TGraph2D();
    nvtx_run_bin[iBin] = new TGraph2D();
    TString gLumiName_temp = ""; gLumiName_temp.Form("lumi_run_%.1f-%.1f",EtaBin[iBin],EtaBin[iBin+1]);
    TString gNvtxName_temp = ""; gNvtxName_temp.Form("nvtx_run_%.1f-%.1f",EtaBin[iBin],EtaBin[iBin+1]);
    lumi_run_bin[iBin]->SetName(gLumiName_temp);
    nvtx_run_bin[iBin]->SetName(gNvtxName_temp);
  }
  ...
  for(Int_t iRun=0; iRun<9; ++iRun) {
    TString NtuplePath = "/u/user/msoh/SE_UserHome/RateStudy2017_v3/SingleMuon/crab_RateStudy2017_v3_SingleMuon_"+Runs[iRun]+"_"+Menu+"_20171108";
  ...
  Int_t    total_bin[nBins] = {0};
  Int_t    count_bin[nBins] = {0};
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++) {
    ...
    if ( currentLS != ev->luminosityBlockNumber ) {
      ...
      if( eventNo!=0 ) {
        ...
        for(Int_t iBin=0; iBin < nBins; ++iBin) {
          if (verbose) cout <<"\teta bin [ "<< EtaBin[iBin] << ", " << EtaBin[iBin+1] << " ]"<< endl;
          if (verbose) cout <<"\t\tcount      = "<<count_bin[iBin]<<endl;
          if (verbose) cout <<"\t\trate@LS    = "<<(double)count_bin[iBin]/23.31<<endl;
          lumi_run_bin[iBin]->SetPoint(nLS-1,currentInstLumi,run,(double)count_bin[iBin]/23.31);
          nvtx_run_bin[iBin]->SetPoint(nLS-1,avePU,run,(double)count_bin[iBin]/23.31);
        }
    ...
    if (ev-> hlt.find(hltname)) {
      ...
      ++count;
      for(Int_t iBin=0; iBin < nBins; ++iBin) {
        if( fabs(eta)>EtaBin[iBin] && fabs(eta)<EtaBin[iBin+1] ) {
          ++total_bin[iBin];
          ++count_bin[iBin];
        }
      }
    ...
    if (eventNo == nentries-1) {
      ...
      for(Int_t iBin=0; iBin < nBins; ++iBin) {
          ...
          if (verbose) cout <<"\t\tcount      = "<<count_bin[iBin]<<endl;
          if (verbose) cout <<"\t\trate@LS    = "<<(double)count_bin[iBin]/23.31<<endl;
          lumi_run_bin[iBin]->SetPoint(nLS,currentInstLumi,run,(double)count_bin[iBin]/23.31);
          nvtx_run_bin[iBin]->SetPoint(nLS,avePU,run,(double)count_bin[iBin]/23.31);
        }
  ...
  for(Int_t iBin=0; iBin < nBins; ++iBin) {
    lumi_run_bin[iBin]->RemovePoint(0);
    nvtx_run_bin[iBin]->RemovePoint(0);
  }
  ...
  cout<<"# LS        = " << nLS << endl;
  cout<<"Rate        = "<<(double)total*SF<<endl;
  for(Int_t iBin=0; iBin < nBins; ++iBin) {
    cout <<"\teta bin [ "<< EtaBin[iBin] << ", " << EtaBin[iBin+1] << " ]"<< endl;
    cout <<"\t\tTotal count = " << total_bin[iBin] << endl;
    cout <<"\t\tRate        = "<<(Double_t)total_bin[iBin]*SF<<endl;
  }
  cout << "||||||||||||||||||||||||||||||" << endl;
  ...
  for(Int_t iBin=0; iBin < nBins; ++iBin) {
    lumi_run_bin[iBin]->Write();
    nvtx_run_bin[iBin]->Write();
  }
