...
void ReadNtuples_Purity_NVTX_v2(TString Period, TString Menu, std::string hltname, bool verbose) {
  ...
  for(Int_t iVTX=0; iVTX<50; ++iVTX) {
    ...
    h_Mass_SS_NVTX[iVTX] = new TH1D("h_Mass_SS_NVTX_"+NVTXBinName,"",10000,0,10000);
  }

  // -- Input & Output files -- //
    TString outputDir = "outputs/Purity_NVTX_v2/v"+theDate;
    if (gSystem->mkdir(outputDir,kTRUE) != -1)
      gSystem->mkdir(outputDir,kTRUE);

    TString outputPath = outputDir+"/Purity_NVTX_v2_"+Period+"_"+Menu+"_"+hltname+"_"+theDate+".root";
  ...
  for(Int_t iVTX=0; iVTX<50; ++iVTX) {
    ...
    h_Mass_NVTX[iVTX]   ->Write();
    h_Mass_OS_NVTX[iVTX]->Write();
    h_Mass_SS_NVTX[iVTX]->Write();
  }
  outfile           -> Close();
