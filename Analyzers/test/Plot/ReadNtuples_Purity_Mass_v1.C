...
void ReadNtuples_Purity_Mass_v1(TString Menu, std::string hltname, bool verbose) {
  ...
  for(Int_t iEta=0; iEta<nEtaBin; ++iEta) {
    EtaBinName.Form("h_Mass_OS_Eta_%.1f-%.1f",EtaBin[iEta],EtaBin[iEta+1]);
    h_Mass_OS_Eta[iEta] = new TH1D("h_Mass_OS_Eta_"+EtaBinName,"",10000,0,10000);
  }

  // -- Input & Output files -- //
    TString outputDir = "outputs/Purity_Mass_v1";
    if (gSystem->mkdir(outputDir,kTRUE) != -1)
      gSystem->mkdir(outputDir,kTRUE);

    TString outputPath = outputDir+"/Purity_Mass_v1_"+Menu+"_"+hltname+"_"+theDate+".root";
    ...
    if ( ev-> hlt.find(hltname) ) {
      ...
      for (std::vector<HLTObjCand>::const_iterator it_mu = ev->hlt.objects.begin(); it_mu != ev->hlt.objects.end(); ++it_mu) {
        if(it_mu->filterTag.find(filter)!=std::string::npos) {
          if (verbose) cout<< "\t\tfilter : " <<it_mu->filterTag<<endl;
          if (verbose) cout << "\t\t" << it_mu->pt << endl;
