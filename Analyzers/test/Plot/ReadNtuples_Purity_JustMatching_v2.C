...
void ReadNtuples_Purity_JustMatching_v2(TString Period, TString Menu, std::string hltname, bool verbose) {
  ...
  // -- Input & Output files -- //
    TString outputDir = "outputs/Purity_JustMatching_v2/v"+theDate;
    if (gSystem->mkdir(outputDir,kTRUE) != -1)
      gSystem->mkdir(outputDir,kTRUE);

    TString outputPath = outputDir+"/Purity_JustMatching_v2_"+Period+"_"+Menu+"_"+hltname+"_"+theDate+".root";
